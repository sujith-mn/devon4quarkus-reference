...
Install all Prerequisites using an automated script, you can find this script in `https://github.com/devonfw-sample/devon4quarkus-reference/azure-terraform/initial_setup_script.sh`
...
*Note*: You need to change `terraform-remote-state` path and `helm charts` path in `terraform-helm-deploy/helm_release.tf` file.
